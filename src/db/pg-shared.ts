...
export function pgDialect(db: PgDb): Dialect {
  const executor: SqlExecutor = {
    async rows<T extends Record<string, unknown>>(query: SQL): Promise<T[]> {
      // node-postgres types the rows through drizzle's Assume<T,
      // QueryResultRow>; the cast only removes that wrapper, T already is the
      // row shape the caller declared.
      ...
      db.transaction(async (tx) => {
        ...
        return await body({
          async rows<T extends Record<string, unknown>>(
            ...
            // Same Assume unwrap as the plain executor above.
