...
/**
 * Registration takes no attestation, so a credential id carries no signature
 * and is chosen by whoever registers. Sign-in looks a credential up by that
 * id and verifies the assertion against whichever row comes back, so a second
 * account claiming a victim's id used to make that lookup a coin toss - and
 * the victim, who has no password and no email recovery, could be locked out
 * permanently. The database is what has to refuse it.
 */
describe("passkey credential ids", () => {
  const insert = (id: string, userId: string, credentialId: string) =>
    app.db
      .prepare(
        `insert into passkey
           (id, public_key, user_id, credential_id, counter, device_type, backed_up)
         values (?1, 'pk', ?2, ?3, 0, 'singleDevice', 0)`,
      )
      .bind(id, userId, credentialId)
      .run();
  ...
  test("still allows one account to hold several distinct credentials", async () => {
    ...
    await app.db
      ...
      .bind(owner, `${owner}@example.test`, now)
      .run();
    ...
    const rows = await app.db
      .prepare("select count(*) as total from passkey where user_id = ?1")
      .bind(owner)
      .first<{ total: number }>();
